...
class BrachioGraph:
    ...
    def __init__(
        ...
        pw_up=1300,                 # pulse-widths for pen up/down
        pw_down=1000,
		filename=None,
    ):
        ...
        Filemode: a = append / w = new
        '''
        logging.basicConfig(level=logging.INFO,
                    format='%(asctime)s %(levelname)-8s %(funcName)-14s' + ' -- ' + '%(message)s',
                    datefmt='%a, %d %b %Y %H:%M:%S',
                    filename='logs/' + filename.split(".")[0] + '.log',
                    filemode='a')

        logging.info('\nXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX BrachioGraph started XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')  
    ...
    def plot_file(self, filename="", wait=0, interpolate=10, bounds=None):
        ...
        logging.info('Filename: ' + filename + '; Wait: ' + str(wait) + '; Interpolate: ' + str(interpolate) + '; Bounds: ' + str(bounds))
    ...
    def draw(self, x=0, y=0, wait=0, interpolate=10):
        ...
        logging.info('draw || x = ' + str(x) + ' y = ' + str(y))  # Comment this out to reduce log size
